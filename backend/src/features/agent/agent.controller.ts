...
class AgentController {
  ...
  /**
   * Get ALL active agents (for employee dropdown in chat)
   */
  public getAllActiveAgents = async (
    req: RequestWithUser,
    res: Response,
    next: NextFunction
  ): Promise<void> => {
    try {
      ...
      const agents = await this.agentService.getAllActiveAgents();
      const agentsResponse = agents.map((agent) => sanitizeAgentResponse(agent));

      res.status(200).json(
        ResponseUtil.success("All active agents retrieved successfully", agentsResponse)
      );
    } catch (error) {
      next(error);
    }
  ...
  public clearTraining = async (
    ...
    try {
      ...
      await this.documentLinkService.clearAllTrainingForAgent(agentId, userId);

      res.status(200).json(
        ResponseUtil.success("Agent training and knowledge cleared successfully")
      );
    } catch (error) {
      next(error);
    }
