...
export class SemanticChunkerService {
  ...
  public async chunkText(
    ...
    try {
      ...
      // Capture positions of raw chunks in original text BEFORE overlap is applied.
      // After applySentenceOverlap() the chunks contain prepended/appended text from
      // neighbouring chunks, so text.indexOf(overlappedChunk) would return -1.
      const rawChunkPositions: Array<{ start: number; end: number }> = [];
      let searchOffset = 0;
      for (const chunk of chunks) {
        const pos = text.indexOf(chunk, searchOffset);
        if (pos >= 0) {
          rawChunkPositions.push({ start: pos, end: pos + chunk.length });
          searchOffset = pos + 1; // Advance to avoid matching the same position twice
        } else {
          // Chunk not found (shouldn't happen for raw chunks) — use -1 as sentinel
          rawChunkPositions.push({ start: -1, end: -1 });
        }
      }

      // Apply simple sentence-based overlap (positions already captured above)
      if (finalConfig.enableOverlap) {
        chunks = this.applySentenceOverlap(chunks, finalConfig);
      }

      // Generate essential metadata using pre-overlap positions
      const metadata = chunks.map((chunk, index) => ({
        chunkIndex: index,
        startPosition: rawChunkPositions[index]?.start ?? -1,
        endPosition: rawChunkPositions[index]?.end ?? -1,
  ...
  private finalizeChunk(chunk: string, config: Required<ChunkingConfig>): string[] {
    ...
    for (const part of result) {
      if (part.length >= config.minChunkSize) {
        finalizedParts.push(part);
      } else if (finalizedParts.length > 0) {
        finalizedParts[finalizedParts.length - 1] += ' ' + part;
      } else {
        finalizedParts.push(part);
      }
    }
